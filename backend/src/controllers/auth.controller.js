...
async function registerCompany(req, res) {
  try {
    const { companyName: rawCompanyName, username, password, email } = req.body;
    ...
    // Atomic safety net: even if two identical company-name registrations
    // race past the findOne check above at the exact same millisecond, the
    // unique index on companyName guarantees only ONE of them can actually
    // be inserted here. The other request gets a clean, safe error instead
    // of silently creating a duplicate/ambiguous company record.
    try {
      await Company.create({ companyName, dbName, ceoEmail: email });
    } catch (dupErr) {
      if (dupErr.code === 11000) {
        return res.status(409).json({ message: 'A company with this name was just registered. Choose a different name.' });
      }
      throw dupErr;
    }
...
async function login(req, res) {
  try {
    const { companyName: rawCompanyName, username, password } = req.body;
    const companyName = (rawCompanyName || '').trim();
    ...
    // Verify the password BEFORE checking isActive, so a deactivated-account
    // message is only ever shown to someone who already proved they know
    // the correct password - this avoids leaking which usernames exist.
    ...
    if (!user.isActive) {
      return res.status(403).json({ message: 'Your account has been deactivated. Please contact your CEO or administrator.' });
    }
